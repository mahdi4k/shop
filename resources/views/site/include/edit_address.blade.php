<form id="address_form_{{ $address->id }}" onsubmit="edit_user_address('{{ $address->id }}');return false;"
      action="{{ url('shop/edit_address').'/'.$address->id }}" method="post">
    {{ csrf_field() }}
        ...
        <tr>
            <td>
                <div class="form-group">
                    ...
                    <select name="ostan_id" class="selectpicker" onchange="get_shahr('edit_ostan_id','edit_shahr_list')"
                            id="edit_ostan_id">
                        <option value="">انتخاب استان</option>
                        @foreach($ostan as $key=>$value)
                            <option @if($address->ostan_id==$value->id) selected="selected"
                                    @endif value="{{ $value->id }}">{{ $value->ostan_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="shahr_id" class="selectpicker" id="edit_shahr_list">
                        @foreach($shahr as $key=>$value)
                            <option @if($key==$address->shahr_id) selected="selected"
