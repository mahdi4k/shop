    ...
    <div class="col-md-5" style="padding-bottom:50px;border-left: 1px solid #cccccc2e;">
        ...
        <div style="margin-top:30px;width:100%;float:right;">
            @if ($product->special==1)
                <div class="products-availability-image"><span >فروش ویژه</span></div>

                @else
            
                <div class="products-availability-image-not"></div>
            @endif
        ...
        <div class="img_box">
            ...
            @if($key<3) <img onclick="change_img('<?= url('upload') . '/' . $value->url ?>')"
            ...
            <div class="single-product-info">
                <div id="product_info">
                    ...
                    <div class="color_box" onclick="set_color('<?= $value->id ?>')">
                        <label style="background:#{{ $value->color_code }}"> @if($key==0) <span class="tick"></span>
                            @endif</label>
                    ...
                    <div class="product-price" style="width:100%;float:right;margin-top: 15px;">
                        ...
                        <p style="font-size:18pt"><span class="product-peice-title">
                                {{ arabic_w2e( number_format($product->price)) }}
                                تومان</span></p>
                        @endif
...
<div class="row content_box" id="tab-custom">
    ...
    <ul class="nav nav-tabs" role="tablist" id="myTabs">
        ...
        <li role="presentation"><a href="#item" aria-controls="item" id="tab_item" role="tab" data-toggle="tab"> مشخصات
                فنی </a></li>
                    ...
                    <ul class="rang_ul">
                        ...
                        <li>
                            ...
                            <div class="rating-container">
                                <div class="bar2">
                                    <div style="width:{{ get_width($score_data['score'][$key],1) }}%" class="holder2">
                                    </div>
                                    @if($score_data['score'][$key]<1) <div class="number_score">
                                        {{ substr($score_data['score'][$key],0,3) }}
                                </div>
                                @endif
                            </div>
                            <div class="bar2">
                                <div style="width:{{ get_width($score_data['score'][$key],2) }}%" class="holder2"></div>
                                @if($score_data['score'][$key]>1 && $score_data['score'][$key]<2) <div
                                    class="number_score">{{ substr($score_data['score'][$key],0,3) }}
                            </div>
                            @endif
                </div>
                <div class="bar2">
                    <div style="width:{{ get_width($score_data['score'][$key],3) }}%" class="holder2"></div>
                    @if($score_data['score'][$key]>=2 && $score_data['score'][$key]<3) <div class="number_score">
                        {{ substr($score_data['score'][$key],0,3) }}
                </div>
                @endif
            </div>
            <div class="bar2">
                <div style="width:{{ get_width($score_data['score'][$key],4) }}%" class="holder2"></div>
                @if($score_data['score'][$key]>=3 && $score_data['score'][$key]<4) <div class="number_score">
                    {{ substr($score_data['score'][$key],0,3)  }}
            </div>
            @endif
        </div>
        <div class="bar2">
            <div style="width:{{ get_width($score_data['score'][$key],5) }}%" class="holder2"></div>
            @if($score_data['score'][$key]>=4 && $score_data['score'][$key]<=5) <div class="number_score">
                {{ substr($score_data['score'][$key],0,3) }}
        </div>
        @endif
    </div>
